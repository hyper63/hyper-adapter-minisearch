...
import { MiniSearch, crocks } from './deps.js'

const { tryCatch, compose } = crocks

const fold = m => m.either(
  e => Promise.reject({ ok: false, message: 'SAL:' + e.message }),
  _ => Promise.resolve({ ok: true })
)

const foldIdentity = m => m.either(
  e => Promise.reject({ ok: false, message: 'SAL:' + e.message }),
  r => Promise.resolve(r)
)

const foldExists = m => m.either(
  e => Promise.reject({ ok: false, message: 'SAL:' + e.message }),
...
export default function () {
  ...
  const create = compose(
    fold,
    tryCatch(({ index, mappings }) => indexes.set(index, new MiniSearch(mappings)))
  )
